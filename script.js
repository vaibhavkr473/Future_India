...
(() => {
    ...
    /* ----------------------------------------------------------
       9. CONTACT FORM — Formspree submission with validation
       ---------------------------------------------------------- */
    const contactForm = document.getElementById('contactForm');
    const formNote = document.getElementById('formNote');
    const contactSubmit = document.getElementById('contactSubmit');
    const FORM_ENDPOINT = 'https://formspree.io/f/xaenyyyq';
    ...
    contactForm?.addEventListener('submit', async (e) => {
        ...
        const originalBtn = contactSubmit.innerHTML;
        ...
        try {
            const response = await fetch(FORM_ENDPOINT, {
                ...
                body: JSON.stringify({
                    ...
                    email,
                    message,
                    _subject: 'New message from Face of Future India',
            ...
            if (response.ok) {
                ...
                contactSubmit.style.background = 'linear-gradient(135deg, #16A34A, #22C55E)';
                formNote.textContent = `Thank you, ${name}! We'll get back to you at ${email} within 24–48 hours.`;
                ...
                setTimeout(() => {
                    contactSubmit.innerHTML = originalBtn;
                ...
                if (response.status === 422 && data.errors) {
                    data.errors.forEach(err => {
                        ...
                        const errEl = contactForm.querySelector(`[data-fs-error="${field}"]`);
                        if (errEl) errEl.textContent = msg;
                    });
                    formNote.textContent = 'Please check the highlighted fields.';
                    formNote.classList.add('error');
                } else if (response.status === 429) {
                    formNote.textContent = 'Too many submissions. Please wait a minute and try again.';
                    formNote.classList.add('error');
                } else {
